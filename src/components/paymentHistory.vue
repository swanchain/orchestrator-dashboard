    ...
    <div class="payment-history container-landing">
      ...
      <el-table v-loading="paymentLoad" :data="paymentData" stripe style="width: 100%">
        ...
        <el-table-column prop="job" label="task uuid" min-width="100">
          <template #default="scope">
            <div class="flex-row center copy-style"
                 @click="system.$commonFun.copyContent(scope.row.task_uuid, 'Copied')">
              {{ system.$commonFun.hiddAddress(scope.row.task_uuid) || '-' }}
              <svg t="1706499607741" class="icon" viewBox="0 0 1024 1024" version="1.1"
                   xmlns="http://www.w3.org/2000/svg" p-id="2309" width="18" height="18">
                <path
        ...
        <el-table-column prop="task_status" label="Task Status" min-width="120">
          <template #default="scope">
            <div>
              ...
              <span v-else-if="scope.row.task_status && scope.row.task_status.toLowerCase() === 'task cancelled'"
                    class="flex-row center">
                {{ scope.row.task_status }} &nbsp;
                <el-popover placement="top" :width="200" effect="dark"
                            popper-style="word-break: break-word; text-align: left;font-size:12px;" trigger="hover"
                            content="Auction waited for more than thirty minutes and could not be assigned.">
                  <template #reference>
                    <div class="flex-row">
                      <svg t="1708417763428" class="icon" viewBox="0 0 1024 1024" version="1.1"
                           xmlns="http://www.w3.org/2000/svg" p-id="7201" width="14" height="14">
              ...
              <span v-else-if="scope.row.task_status && scope.row.task_status.toLowerCase() === 'task terminating'"
                    class="flex-row center">
                {{ scope.row.task_status }} &nbsp;
                <el-popover placement="top" :width="200" effect="dark"
                            popper-style="word-break: break-word; text-align: left;font-size:12px;" trigger="hover"
                            content="The user initiated an early termination request and is waiting in the early termination queue for the task to be terminated">
                  <template #reference>
                    <div class="flex-row">
                      <svg t="1708417763428" class="icon" viewBox="0 0 1024 1024" version="1.1"
                           xmlns="http://www.w3.org/2000/svg" p-id="7201" width="14" height="14">
        ...
        <el-table-column prop="payment_status" label="payment status" min-width="130">
          <template #default="scope">
            <div>
              ...
              <span v-if="scope.row.status && scope.row.status.toLowerCase() === 'reward claimed'"
                    class="flex-row center">
                {{ scope.row.status }} &nbsp;
                <el-popover placement="top" :width="200" effect="dark"
                            popper-style="word-break: break-word; text-align: left;font-size:12px;" trigger="hover"
                            content="Rewards have been claimed or have been automatically distributed to the CP's wallet">
                  <template #reference>
                    <div class="flex-row">
                      <svg t="1708417763428" class="icon" viewBox="0 0 1024 1024" version="1.1"
                           xmlns="http://www.w3.org/2000/svg" p-id="7201" width="14" height="14">
              ...
              <span v-else-if="scope.row.status && scope.row.status.toLowerCase() === 'cooling down'"
                    class="flex-row center">
                {{ scope.row.status }} &nbsp;
                <el-popover placement="top" :width="200" effect="dark"
                            popper-style="word-break: break-word; text-align: left;font-size:12px;" trigger="hover"
                            content="Waiting for 3 day cooling down, you will be able to claim the reward if you pass the claim review">
                  <template #reference>
                    <div class="flex-row">
                      <svg t="1708417763428" class="icon" viewBox="0 0 1024 1024" version="1.1"
                           xmlns="http://www.w3.org/2000/svg" p-id="7201" width="14" height="14">
              ...
              <span v-else-if="scope.row.status && scope.row.status.toLowerCase() === 'pending'"
                    class="flex-row center">
                {{ scope.row.status }} &nbsp;
                <el-popover placement="top" :width="200" effect="dark"
                            popper-style="word-break: break-word; text-align: left;font-size:12px;" trigger="hover"
                            content="This task is still running, you can get your reward after task finished ">
                  <template #reference>
                    <div class="flex-row">
                      <svg t="1708417763428" class="icon" viewBox="0 0 1024 1024" version="1.1"
                           xmlns="http://www.w3.org/2000/svg" p-id="7201" width="14" height="14">
              ...
              <span v-else-if="scope.row.status && scope.row.status.toLowerCase() === 'claim failed'"
                    class="flex-row center">
                {{ scope.row.status }} &nbsp;
                <el-popover placement="top" :width="200" effect="dark"
                            popper-style="word-break: break-word; text-align: left;font-size:12px;" trigger="hover"
                            content="Cannot claim this reward because of some reason">
                  <template #reference>
                    <div class="flex-row">
                      <svg t="1708417763428" class="icon" viewBox="0 0 1024 1024" version="1.1"
                           xmlns="http://www.w3.org/2000/svg" p-id="7201" width="14" height="14">
              ...
              <el-button type="primary" v-else-if="scope.row.status && scope.row.status.toLowerCase() === 'rewardable'"
                         plain @click="rewardFun(scope.row, 1)">Claim Reward
              </el-button>
        ...
        <el-table-column prop="transaction_hash" label="transaction hash" min-width="110">
          <template #default="scope">
            <span>{{
                scope.row.amount && scope.row.amount.indexOf('-') > -1 ? '-' : scope.row.transaction_hash || '-'
              }}</span>
          </template>
        </el-table-column>
      ...
      <el-pagination class="flex-row" :page-size="pagin.pageSize" :current-page="pagin.pageNo" :pager-count="5"
                     :small="small" :background="background" layout="total, prev, pager, next" :total="pagin.total"
                     @size-change="handleSizeChange" @current-change="handleCurrentChange"
      />
    </div>

    <el-dialog v-model="txhashVisible" title="Payment Detail" :width="bodyWidth" :append-to-body="false"
               custom-class="wrongNet" :before-close="handleClose">
      <div class="cont">
          ...
          <div class="fast width">
            ...
            <div class="address">
              <div class="flex-row hash">
                ...
                <svg @click="system.$commonFun.copyContent(txHash, 'Copied')" t="1706499607741" class="icon"
                     viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2309" width="18"
                     height="18">
                  <path
                      d="M720 192h-544A80.096 80.096 0 0 0 96 272v608C96 924.128 131.904 960 176 960h544c44.128 0 80-35.872 80-80v-608C800 227.904 764.128 192 720 192z m16 688c0 8.8-7.2 16-16 16h-544a16 16 0 0 1-16-16v-608a16 16 0 0 1 16-16h544a16 16 0 0 1 16 16v608z"
                      p-id="2310" fill="#b5b7c8"></path>
